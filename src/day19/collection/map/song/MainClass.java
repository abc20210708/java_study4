package day19.collection.map.song;
...
public class MainClass {
    ...
    public static void insertMusic() {
        ...
        if (!artists.containsKey(artist)) { //신규 가수 등록
            songs = new HashSet<>();///노래리스트 만들고 null인 songs에 넣어줌!
            songs.add(song);
            artists.put(artist,songs);
            System.out.println("# 아티스트" + artist + "님이 신규등록되었습니다.");

        } else if (artists.containsKey(artist)) { ///이미 가수가 등록된 경우
            ///노래만 추가 100번지에
            songs = artists.get(artist);
            if(songs.add(song)) {
                System.out.println("# 아티스트" + artist + "님의 노래목록에 " + song + "이(가) 추가되었습니다.");
    ...
    public static void searchMusic() {
        ...
        if (artists.containsKey(artist)) {
            System.out.println("* " +artist + "님의 노래목록 *");
    ...
    private static void saveData() {
        ...
        FileOutputStream fos = null;///텍스트만 저아
